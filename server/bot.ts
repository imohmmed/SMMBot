...
async function sendMainMenu(chatId: number, messageId?: number, firstName?: string) {
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [{ text: "مراسلة الدعم", url: "tg://user?id=977560967", style: "success", icon_custom_emoji_id: "5472107610087889157" }],
    ] as any,
  };
...
async function showServices(chatId: number, messageId?: number) {
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [{ text: "اشتراكات", callback_data: "service_type_subscriptions", style: "primary", icon_custom_emoji_id: "5465186493628685874" }],
...
async function showServiceTypeCategories(chatId: number, type: string, messageId?: number) {
  ...
  if (cats.length === 0) {
    ...
    const keyboard = { inline_keyboard: [[{ text: "رجوع", callback_data: "services", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] };
    if (messageId) {
  ...
  if (isSubscription) {
    ...
    keyboard = {
      inline_keyboard: [
        ...
        [{ text: "رجوع", callback_data: `cat_${svc.categoryId}`, style: "danger", icon_custom_emoji_id: "5875082500023258804" }],
      ] as any,
    };
...
async function showPaymentMethod(chatId: number, methodId: number, telegramId: string, messageId?: number) {
  ...
  if (isUsdt) {
    buttons = [
      [
        { text: "$10", callback_data: `usdtamount_${methodId}_10`, style: "primary" },
        { text: "$20", callback_data: `usdtamount_${methodId}_20`, style: "primary" },
      ],
      [
        { text: "$50", callback_data: `usdtamount_${methodId}_50`, style: "primary" },
        { text: "$100", callback_data: `usdtamount_${methodId}_100`, style: "primary" },
      ],
      [
        { text: "مبلغ اختياري بالدولار", callback_data: `custom_amount_${methodId}`, style: "success" },
      ],
    ...
    buttons = [
      ...
      [
        ...
        { text: "75,000 IQD", callback_data: `amount_${methodId}_75000`, style: "primary" },
      ],
      [
...
async function showMyOrders(chatId: number, telegramId: string, messageId?: number) {
  ...
  if (userOrders.length === 0) {
    ...
    const keyboard = { inline_keyboard: [[{ text: "القائمة الرئيسية", callback_data: "main_menu", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] };
    if (messageId) {
      return bot.editMessageText(text, { chat_id: chatId, message_id: messageId, reply_markup: keyboard });
    }
    return bot.sendMessage(chatId, text, { reply_markup: keyboard });
  ...
  buttons.push([{ text: "القائمة الرئيسية", callback_data: "main_menu", style: "danger", icon_custom_emoji_id: "5875082500023258804" }]);
...
async function showAdminStats(chatId: number, messageId?: number) {
  ...
  const keyboard = { inline_keyboard: [[{ text: "رجوع", callback_data: "admin_panel", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] };
  if (messageId) {
...
async function showDeleteCategoryServices(chatId: number, slug: string, messageId?: number) {
  ...
  if (svcs.length === 0) {
    ...
    const keyboard = { inline_keyboard: [[{ text: "رجوع", callback_data: "admin_delete_services", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] };
    if (messageId) {
      return bot.editMessageText(text, { chat_id: chatId, message_id: messageId, parse_mode: "Markdown", reply_markup: keyboard });
    }
    return bot.sendMessage(chatId, text, { parse_mode: "Markdown", reply_markup: keyboard });
  ...
  buttons.push([{ text: "رجوع", callback_data: "admin_delete_services", style: "danger", icon_custom_emoji_id: "5875082500023258804" }]);
...
async function showAdminMargin(chatId: number, messageId?: number) {
  ...
  const marginKeyboard = { inline_keyboard: [[{ text: "رجوع", callback_data: "admin_panel", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] };
  ...
  try {
    await bot.editMessageText(
      ...
      `❌ فشل: ${failed}`,
      {
        ...
        parse_mode: "Markdown",
        reply_markup: { inline_keyboard: [[{ text: "لوحة الإدارة", callback_data: "admin_panel", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] },
      }
    );
  } catch {}
  ...
  bot.on("callback_query", async (query) => {
    ...
    try {
        ...
        try {
          ...
          await bot.sendMessage(
            ...
            `💰 رصيدك المتبقي: ${formatNumber(parseFloat(user.balance) - oState.price)} IQD`,
            {
              parse_mode: "Markdown",
              reply_markup: {
                inline_keyboard: [[{ text: "القائمة الرئيسية", callback_data: "main_menu", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any],
              },
            }
          );
      ...
      if (data.startsWith("custom_amount_")) {
        ...
        setState(telegramId, { step: "custom_amount", methodId, isUsdt });
        if (isUsdt) {
          return bot.sendMessage(chatId, "💵 أرسل المبلغ المطلوب بالدولار ($):", {
            parse_mode: "Markdown",
          });
        }
        return bot.sendMessage(chatId, "💵 أرسل المبلغ المطلوب (بالدينار العراقي):", {
          parse_mode: "Markdown",
        });
      }

      // USDT preset amount (in dollars, convert to IQD)
      if (data.startsWith("usdtamount_")) {
        const parts = data.split("_");
        const methodId = parseInt(parts[1]);
        const usdAmount = parseInt(parts[2]);
        const iqdAmount = usdAmount * USD_TO_IQD;
        setState(telegramId, { step: "deposit_screenshot", amount: iqdAmount, methodId });
        return bot.sendMessage(
          chatId,
          `✅ المبلغ: *$${usdAmount}* = *${formatNumber(iqdAmount)} IQD*\n\nالآن أرسل *سكرين شوت* لعملية التحويل:`,
          { parse_mode: "Markdown" }
        );
      }
      ...
      if (data === "broadcast_text") {
        ...
        return bot.editMessageText("📝 أرسل نص الإذاعة:", {
          ...
          message_id: messageId,
          reply_markup: {
            inline_keyboard: [[{ text: "رجوع", callback_data: "admin_broadcast", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any],
          },
        });
      }
      ...
      if (data === "provider_custom") {
        ...
        buttons.push([{ text: "رجوع", callback_data: "admin_add_service", style: "danger", icon_custom_emoji_id: "5875082500023258804" }]);
      ...
      if (data === "provider_subscription") {
        ...
        if (subCats.length === 0) {
          return bot.editMessageText("❌ لا توجد أقسام اشتراكات. أضف قسم اشتراكات أولاً من إدارة الأقسام.", {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: { inline_keyboard: [[{ text: "رجوع", callback_data: "admin_add_service", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any] },
          });
        }
            ...
            `هذا الإجراء لا يمكن التراجع عنه!`,
            {
              ...
              reply_markup: {
                inline_keyboard: [
                  ...
                  [{ text: "لا، رجوع", callback_data: "admin_delete_services", style: "danger", icon_custom_emoji_id: "5875082500023258804" }],
                ] as any,
              },
            }
          );
        ...
        if (svc) {
          ...
          await bot.editMessageText(
            `✅ تم حذف الخدمة "${svc.name}" بنجاح!`,
            {
              ...
              message_id: messageId,
              reply_markup: {
                inline_keyboard: [[{ text: "رجوع", callback_data: "admin_delete_services", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any],
              },
            }
          );
        }
      ...
      if (data.startsWith("assign_cat_")) {
        ...
        return bot.editMessageText(
          ...
          `💵 السعر: ${formatNumber(parseFloat(serviceInfo.rate))} IQD (+ ${margin}% ربح)`,
          {
            ...
            parse_mode: "Markdown",
            reply_markup: {
              inline_keyboard: [[{ text: "لوحة الإدارة", callback_data: "admin_panel", style: "danger", icon_custom_emoji_id: "5875082500023258804" }] as any],
            },
          }
        );
      }
  ...
  bot.on("message", async (msg) => {
    ...
    try {
      ...
      if (state.step === "custom_amount" && msg.text) {
        const rawAmount = parseInt(msg.text.replace(/[,$]/g, ""));
        if (state.isUsdt) {
          if (isNaN(rawAmount) || rawAmount < 1) {
            return bot.sendMessage(chatId, "❌ المبلغ غير صحيح. أدخل مبلغ صحيح بالدولار");
          }
          const iqdAmount = rawAmount * USD_TO_IQD;
          setState(telegramId, { step: "deposit_screenshot", amount: iqdAmount, methodId: state.methodId });
          return bot.sendMessage(
            chatId,
            `✅ المبلغ: *$${rawAmount}* = *${formatNumber(iqdAmount)} IQD*\n\nالآن أرسل *سكرين شوت* لعملية التحويل:`,
            { parse_mode: "Markdown" }
          );
        }
      ...
      if (state.step === "deposit_recharge_code" && msg.text) {
        const rechargeCode = msg.text.replace(/[-\s]/g, "").trim();
        if (!rechargeCode || rechargeCode.length < 3) {
          return bot.sendMessage(chatId, "❌ الرقم غير صحيح. أرسل رقم التعبئة بدون فوارز:");
        }
        setState(telegramId, { step: "deposit_screenshot", amount: state.amount, methodId: state.methodId, rechargeCode });
        return bot.sendMessage(
          chatId,
          `✅ رقم التعبئة: *${rechargeCode}*\n\nالآن أرسل *سكرين شوت* لعملية التحويل:`,
          { parse_mode: "Markdown" }
        );
      }
      ...
      if (state.step === "deposit_screenshot" && msg.photo) {
        ...
        const method = await storage.getPaymentMethod(state.methodId);
        const rechargeCode = state.rechargeCode || null;
        ...
        const deposit = await storage.createDeposit({
          ...
          adminNote: rechargeCode ? `رقم التعبئة: ${rechargeCode}` : null,
        });
        ...
        await bot.sendMessage(
          ...
          `المبلغ: ${formatNumber(state.amount)} IQD\n` +
          `الطريقة: ${method?.name}\n` +
          (rechargeCode ? `رقم التعبئة: ${rechargeCode}\n` : ``) +
          `رقم العملية: #${deposit.id}\n\n` +
          `سيتم مراجعة طلبك قريباً.`,
          { parse_mode: "Markdown" }
        );
